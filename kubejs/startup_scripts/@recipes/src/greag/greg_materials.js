GTCEuStartupEvents.registry('gtceu:material', event => {
    ...
    event.create('hepatizon').ingot(1).fluid().components('3x copper', '1x cobalt', '3x obsidian').color(0x4a0e66).iconSet(SHINY).flags(foil, gear, long_rod, plates, rod, rotor, small_gear).rotorStats(150, 120, 5.5, 35500).blastTemp(1500, 'high')//.toolStats(9, 6, 500, 4)
    ...
    //event.create('stone').flags(frame, plates, bolt_and_screw).color(0xa9a3a3)

    // event.create('signalum_120').ingot(1).fluid().components('1x silver', '3x copper', '4x redstone').color(0xf86014).secondaryColor(0x8a0003).iconSet(SHINY).blastTemp(1700, 'low', mv, 1200).flags(foil, gear, long_rod, plates, rod, rotor, small_gear, ring, frame).rotorStats(200, 150, 5.5, 20900)
    // event.create('lumium_89').ingot(1).fluid().components('1x silver', '3x tin', '2x glowstone').color(0xffffb3).secondaryColor(0xfceea8).iconSet(SHINY).blastTemp(1700, 'low', hv, 1500).flags(foil, gear, long_rod, plates, rod, rotor, small_gear, ring, frame).rotorStats(200, 150, 5.5, 26800)
    // event.create('enderium_100').ingot(1).fluid().components('3x lead', '1x diamond', '2x ender_pearl').color(0x006666).secondaryColor(0x0a2f56).iconSet(SHINY).blastTemp(3500, 'low', ev, 1500).flags(foil, gear, long_rod, plates, rod, rotor, small_gear, ring, frame).rotorStats(200, 180, 5.5, 33600)


    event.create('signalum').ingot(1).fluid().components('1x silver', '3x copper', '4x redstone').color(0xff3300).iconSet(SHINY).blastTemp(1700, 'low', va[mv], 1200).flags(foil, gear, long_rod, plates, rod, rotor, small_gear, ring, frame).cableProperties(v[mv], 16, 0, true);
    event.create('lumium').ingot(1).fluid().components('1x silver', '3x tin', '2x glowstone').color(0xffffb3).iconSet(SHINY).blastTemp(1700, 'low', va[hv], 1500).flags(foil, gear, long_rod, plates, rod, rotor, small_gear, ring, frame).cableProperties(v[hv], 16, 0, true);
    event.create('enderium').ingot(1).fluid().components('3x lead', '1x diamond', '2x ender_pearl').color(0x006666).iconSet(SHINY).blastTemp(3500, 'low', va[ev], 1500).flags(foil, gear, long_rod, plates, rod, rotor, small_gear, ring, frame).cableProperties(v[ev], 32, 0, true);

    GTMaterials.WroughtIron.addFlags(frame);
    GTMaterials.TreatedWood.addFlags(bolt_and_screw, rod)
    })